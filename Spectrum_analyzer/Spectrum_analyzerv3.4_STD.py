...
class RamanFitterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Cold Atom Raman Analysis - v6.0 (Presets & Full Export)")
        ...
        self.last_peak_names = []
        self.last_fit_x = None      # 存储最后一次拟合的X数据
        self.last_fit_y_raw = None  # 存储最后一次拟合的Y原始数据
        ...
        # --- 0. 预设管理 (New Position: Top) ---
        self.add_section_label(self.frame_left, "0. Presets (Config)")
        f_preset = ttk.Frame(self.frame_left)
        f_preset.pack(fill=tk.X, pady=2)
        ttk.Button(f_preset, text="Save Preset", command=self.save_preset).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,5))
        ttk.Button(f_preset, text="Load Preset", command=self.load_preset).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(5,0))
        ...
        # Peak Naming
        ttk.Label(self.frame_left, text="Peak Names (comma separated):", font=("Arial", 9)).pack(anchor="w", pady=(5,0))
        self.entry_peak_names = ttk.Entry(self.frame_left)
        self.entry_peak_names.pack(fill=tk.X, pady=2)
        self.entry_peak_names.insert(0, "Left Atom, Right Atom")
        
        self.check_show_labels_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(self.frame_left, text="Show Labels on Plot", variable=self.check_show_labels_var).pack(anchor="w")
        ...
        self.entry_xlabel.insert(0, "Frequency (kHz)")
        ...
        self.btn_export = ttk.Button(self.frame_left, text="Export Full Data (CSV)", command=self.export_full_data, state=tk.DISABLED)
        self.btn_export.pack(fill=tk.X, pady=15)

        # --- 结果表格 ---
        self.tree_frame = ttk.Frame(self.frame_left)
        self.tree_frame.pack(fill=tk.BOTH, expand=True)
        columns = ("Name", "Center", "Width", "Area")
        self.tree = ttk.Treeview(self.tree_frame, columns=columns, show="headings", height=6)
        
        self.tree.heading("Name", text="Name"); self.tree.column("Name", width=80, anchor="center")
        self.tree.heading("Center", text="Center"); self.tree.column("Center", width=60, anchor="center")
        self.tree.heading("Width", text="Width"); self.tree.column("Width", width=60, anchor="center")
        self.tree.heading("Area", text="Area"); self.tree.column("Area", width=60, anchor="center")
    ...
    def get_peak_names(self, num_peaks):
        raw_names = self.entry_peak_names.get().split(',')
        names = [n.strip() for n in raw_names if n.strip()]
        while len(names) < num_peaks:
            names.append(f"Peak {len(names)+1}")
        return names[:num_peaks]
    ...
    def perform_fit(self):
        ...
        try:
            ...
            self.last_peak_names = self.get_peak_names(num_peaks)
            self.last_fit_x = x
            self.last_fit_y_raw = y
            
            self.btn_export.config(state=tk.NORMAL)
            
            self.update_results_tree(full_params, num_peaks, self.last_peak_names)
            self.plot_fit_result(x, y, full_params, num_peaks, tgt, src, self.last_peak_names)
        except Exception as e:
            messagebox.showerror("Fit Error", str(e))

    def update_results_tree(self, params, num_peaks, names):
        ...
        for i in range(num_peaks):
            ...
            self.tree.insert("", "end", values=(names[i], f"{params[base+1]:.4f}", f"{abs(params[base+2]):.4f}", f"{area:.4f}"))

    def plot_fit_result(self, x, y, params, num_peaks, tgt, src, names):
        self.ax.clear()
        ...
        show_labels = self.check_show_labels_var.get()
        
        for i in range(num_peaks):
            base = 1 + i*3
            amp, cen, sig = params[base], params[base+1], params[base+2]
            y_sub = self.multi_gaussian(x_s, *([offset, amp, cen, sig]))
            self.ax.plot(x_s, y_sub, '--', color=colors[i%5], lw=2, label=names[i])
            self.ax.fill_between(x_s, offset, y_sub, color=colors[i%5], alpha=0.2)
            
            if show_labels:
                label_y = offset + amp + (np.max(y) - np.min(y))*0.05 
                self.ax.text(cen, label_y, names[i], ha='center', va='bottom', fontsize=11, fontweight='bold', color=colors[i%5])
    ...
    def save_preset(self):
        preset = {
            "x_map_enable": self.check_x_map_var.get(),
            "x_old_min": self.entry_x_old_min.get(),
            "x_old_max": self.entry_x_old_max.get(),
            "x_new_min": self.entry_x_new_min.get(),
            "x_new_max": self.entry_x_new_max.get(),
            "y_map_enable": self.check_y_map_var.get(),
            ...
            "baseline_val": self.entry_baseline.get(),
            "peak_names": self.entry_peak_names.get(),
            "show_labels": self.check_show_labels_var.get(),
            "title": self.entry_plot_title.get(),
            "xlabel": self.entry_xlabel.get(),
            "ylabel": self.entry_ylabel.get()
        }
        
        fpath = filedialog.asksaveasfilename(defaultextension=".json", filetypes=[("JSON Config", "*.json")])
        if fpath:
            try:
    ...
    def load_preset(self):
        fpath = filedialog.askopenfilename(filetypes=[("JSON Config", "*.json")])
        if fpath:
            try:
                ...
                # Apply settings
                self.check_x_map_var.set(preset.get("x_map_enable", False))
                self.entry_x_old_min.delete(0, tk.END); self.entry_x_old_min.insert(0, preset.get("x_old_min", ""))
                self.entry_x_old_max.delete(0, tk.END); self.entry_x_old_max.insert(0, preset.get("x_old_max", ""))
                self.entry_x_new_min.delete(0, tk.END); self.entry_x_new_min.insert(0, preset.get("x_new_min", ""))
                self.entry_x_new_max.delete(0, tk.END); self.entry_x_new_max.insert(0, preset.get("x_new_max", ""))
                
                self.check_y_map_var.set(preset.get("y_map_enable", False))
                self.entry_y_scale.delete(0, tk.END); self.entry_y_scale.insert(0, preset.get("y_scale", "1.0"))
                self.entry_y_offset.delete(0, tk.END); self.entry_y_offset.insert(0, preset.get("y_offset", "0.0"))
                
                self.num_peaks_var.set(preset.get("num_peaks", 1))
                self.baseline_mode.set(preset.get("baseline_mode", "Auto"))
                self.entry_baseline.delete(0, tk.END); self.entry_baseline.insert(0, preset.get("baseline_val", "0.0"))
                self.entry_peak_names.delete(0, tk.END); self.entry_peak_names.insert(0, preset.get("peak_names", ""))
                self.check_show_labels_var.set(preset.get("show_labels", True))
                
                self.entry_plot_title.delete(0, tk.END); self.entry_plot_title.insert(0, preset.get("title", ""))
                self.entry_xlabel.delete(0, tk.END); self.entry_xlabel.insert(0, preset.get("xlabel", ""))
                self.entry_ylabel.delete(0, tk.END); self.entry_ylabel.insert(0, preset.get("ylabel", ""))
    ...
    def export_full_data(self):
        ...
        try:
            with open(file_path, mode='w', newline='') as f:
                ...
                writer.writerow(["Name", "Amplitude", "Center", "Width (Sigma)", "Area"])
                ...
                for i in range(self.last_fit_num_peaks):
                    ...
                    writer.writerow([self.last_peak_names[i], amp, cen, sig, area])
                
                writer.writerow([])
                
                # --- Part 2: Curve Data ---
                writer.writerow(["=== CURVE DATA ==="])
                
                # Prepare headers
                headers = ["X_Mapped", "Y_Raw", "Y_Fit_Total"]
                for i in range(self.last_fit_num_peaks):
                    headers.append(f"Y_{self.last_peak_names[i]}")
                writer.writerow(headers)
                
                # Calculate curves
                x = self.last_fit_x
                y_raw = self.last_fit_y_raw
                y_total = self.multi_gaussian(x, *self.last_fit_params)
                
                offset = self.last_fit_params[0]
                
                # Build rows
                for i in range(len(x)):
                    row = [x[i], y_raw[i], y_total[i]]
                    
                    # Add individual peak values (including offset for plotting convenience, or just the peak component)
                    # Here we add (Component + Offset) to represent what is drawn in the plot
                    for p in range(self.last_fit_num_peaks):
                        base = 1 + p*3
                        amp = self.last_fit_params[base]
                        cen = self.last_fit_params[base+1]
                        sig = self.last_fit_params[base+2]
                        y_comp = self.multi_gaussian(np.array([x[i]]), *([offset, amp, cen, sig]))[0]
                        row.append(y_comp)
                        
                    writer.writerow(row)
                    
            messagebox.showinfo("Export", f"Full data saved to:\n{file_path}")
        except Exception as e:

...
\begin{itemize}
  ...
  \item extract a common differential light shift from a measured counter-propagating Raman
  peak pair and report the peak centers after removing that shift,
...
$v_x$ from two signed detuning measurements. It also provides a quick experimental light-shift
correction based on the mean of a measured $+k_{\mathrm{eff}}$/$-k_{\mathrm{eff}}$ peak pair.
...
\subsection{Differential Light-shift Extraction and Correction}

For a fixed transition direction and a pair of counter-propagating Raman resonances measured with
the same Raman pulse and optical-power configuration, let
$\delta_{+}^{\mathrm{meas}}$ and $\delta_{-}^{\mathrm{meas}}$ denote the fitted centers of the
$+k_{\mathrm{eff}}$ and $-k_{\mathrm{eff}}$ peaks. The quick correction models them as

\begin{align}
\delta_{+}^{\mathrm{meas}} &= +D + s_{\mathrm{tr}}\omega_r + \delta_{\mathrm{AC}}, \\
\delta_{-}^{\mathrm{meas}} &= -D + s_{\mathrm{tr}}\omega_r + \delta_{\mathrm{AC}},
\end{align}

where $D$ is the signed Doppler term and $\delta_{\mathrm{AC}}$ is the common differential
AC Stark shift. Adding and subtracting the two equations gives

\begin{equation}
D = \frac{\delta_{+}^{\mathrm{meas}}-\delta_{-}^{\mathrm{meas}}}{2},
\end{equation}

\begin{equation}
\delta_{\mathrm{AC}}
= \frac{\delta_{+}^{\mathrm{meas}}+\delta_{-}^{\mathrm{meas}}}{2}
-s_{\mathrm{tr}}\omega_r.
\end{equation}

The peak centers with this common light shift removed are

\begin{align}
\delta_{+}^{(0)} &= \delta_{+}^{\mathrm{meas}}-\delta_{\mathrm{AC}}, \\
\delta_{-}^{(0)} &= \delta_{-}^{\mathrm{meas}}-\delta_{\mathrm{AC}}.
\end{align}

For the central co-propagating Raman resonance, the effective wave vector is approximated as
$k_{\mathrm{eff,co}}\simeq 0$. Its Doppler and recoil terms are therefore neglected in this quick
model, giving

\begin{equation}
\delta_{\mathrm{co}}^{\mathrm{meas}}=\delta_{\mathrm{AC}},
\qquad
\delta_{\mathrm{co}}^{(0)}=0.
\end{equation}

Their corrected mean provides an immediate consistency check:

\begin{equation}
\frac{\delta_{+}^{(0)}+\delta_{-}^{(0)}}{2}=s_{\mathrm{tr}}\omega_r.
\end{equation}

These equations may be used with angular frequencies or ordinary frequencies as long as every
term uses the same convention. The GUI accepts and reports ordinary-frequency values in kHz, so
its signed recoil center is $s_{\mathrm{tr}}f_r$.

This feature intentionally neglects Zeeman shifts and frequency-reference or scan-zero offsets.
It also assumes that the two counter-propagating peaks have the same differential light shift.
Any neglected common offset is therefore absorbed into the reported $\delta_{\mathrm{AC}}$. For a
meaningful correction, both input peaks must correspond to the same $m_F$ transition and must be
recorded with the same Raman pulse, powers, polarization, and retro-reflection configuration.
...
\begin{itemize}
  ...
  calibration of $\alpha$ and $v_x$ from scan results, together with the measured-pair
  differential light-shift correction.
\end{itemize}
...
Measured $\delta_+$ center & kHz & Fitted center of the $+k_{\mathrm{eff}}$ counter-propagating Raman peak. \\
Measured $\delta_-$ center & kHz & Fitted center of the $-k_{\mathrm{eff}}$ counter-propagating Raman peak. \\
Light-shift transition & --- & Selects the sign of the recoil center used in the correction. \\
...
The \texttt{Detuning} page provides four text-based result areas:
...
\begin{itemize}
  ...
  \item \texttt{Light-shift Result}: reports the measured pair mean, signed recoil center,
  extracted differential light shift, half-splitting Doppler term, and both peak centers after
  removal of the common light shift, together with the predicted measured and corrected centers
  of the central co-propagating resonance. These quantities use Matplotlib mathematical typesetting
  rather than platform-dependent Unicode subscript and superscript glyphs. The same tab plots a
  normalized three-peak spectrum containing the two counter-propagating resonances and the central
  co-propagating resonance, together with its light-shift-corrected counterpart; the peak centers are calculated exactly, while the
  displayed linewidth and amplitude are schematic rather than a fit to raw samples. The plot
  supports coordinate hover, mouse-wheel zoom, toolbar pan and box zoom, Home reset, and figure
  export. A draggable divider reallocates space between the numerical summary and plot,
